                ...
                <section id="widgets-Statistics" class="mr-1 ml-1 mb-1">
                    <div class="row">
                        <div class="col-md-6  mt-2 mb-2">
                            <h4>Estimate Information Update</h4>
                        </div>
                    </div>
                    <div class="row" style="padding-left: 10px; padding-right: 10px">
                        <div class="col-12">
                            <form action="{{ route('estimate-list.update', $estimateInfo->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                        ...
                                        <div class="col-md-2 changeColStyle">
                                            ...
                                            <select name="customer_id" class="common-select2" style="width: 100% !important" id="" required>
                                                ...
                                                @foreach ($partys as $party)
                                                    <option value="{{ $party->id }}" {{$estimateInfo->customer_id == $party->id ? 'selected':''}}> {{ $party->pi_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-2 changeColStyle">
                                            <label>Estimate Date</label>
                                            <input type="date" class="form-control inputFieldHeight" name="estimate_date" required value="{{$estimateInfo->estimate_date}}">
                                        </div>
                                        <div class="col-md-2 changeColStyle">
                                            <label>Expiry Date</label>
                                            <input type="date" class="form-control inputFieldHeight" name="expire_date" required value="{{$estimateInfo->expire_date}}">
                                        </div>
                                        <div class="col-md-2 changeColStyle">
                                            <label>Estimate Amount</label>
                                            <input type="number" class="form-control inputFieldHeight" name="estimate_amount" required value="{{$estimateInfo->estimate_amount}}">
                                        </div>
                                        <div class="col-md-2 changeColStyle">
                                            <label>Estimate Number</label>
                                            <input type="text" class="form-control inputFieldHeight" name="estimate_number" required value="{{$estimateInfo->estimate_number}}">
                                        </div>
                                        <div class="col-md-2 changeColStyle">
                                            <label>Reference</label>
                                            <input type="text" class="form-control inputFieldHeight" name="reference" required value="{{$estimateInfo->reference}}">
                                        </div>
