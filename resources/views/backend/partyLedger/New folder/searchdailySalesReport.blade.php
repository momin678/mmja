                    ...
                    <div class="row pt-2">
                        ...
                        <table   class="table table-sm table-bordered">
                            ...
                            @foreach (App\ItemList::where('group_no','>=', 11)->where('group_no','<=',17)->select('style_id')->distinct()->get() as $it)
                                ...
                                @endphp
                                @if($style->colorSaleCount($date,$style->id)>0)
                                <tr>
                                    <td rowspan="{{ $style->colorSaleCount($date,$style->id) }}"  colspan="2">{{ $style->style_name }}</td>
                                    @php
                                    ...
                                    @if($row==0)
                                        ...
                                        <td>{{ $itmXxxlC }}</td>
                                        <td>{{ $tqty=$colorQty }}</td>
                                        <td>{{$sr=number_format((float)$brand->colorItemSaleRateDate($style,$brand,$date), 3,'.','') }}</td>
                        ...
                        @foreach (App\ItemList::where('group_no','>', 17)->where('group_no','<',24)->select('style_id')->distinct()->get() as $it)
                            ...
                            @if($style->colorSaleCount($date,$style->id)>0)
                            <tr>
                                <td rowspan="{{ $style->colorSaleCount($date,$style->id) }}"  colspan="3">{{ $style->style_name }}</td>
                                @foreach (App\ItemList::select('brand_id')->where('style_id',$it->style_id)->distinct()->get()
                                    as $color)
                                    @php
