            ...
            <div class="content-body">
                ...
                <section id="widgets-Statistics">
                    <div class="row">
                        <div class="col-md-4">
                            {{-- {{ date('d M Y') }} --}}
                            <h4>General Ledger {{ $from}} to {{$to}}</h4>
                        </div>
                    ...
                    <div class="row pt-2">
                        <div class="col-md-12">
                            <a href="{{ route('print-gl-date-range',[$from,$to]) }}" class="btn btn-sm btn-info float-right"
                        ...
                        <table   class="table table-sm table-bordered">
                            ...
                            @foreach (App\JournalRecordsTemp::whereBetween('journal_date', [$from,$to])->distinct()->get('account_head_id') as $unique_acc_head)
                            <tr>                                
                            ...
                            <tr>                                
                                ...
                                @php
                                    ...
                                    $each_ledger_cr=0;
                                @endphp
                                @foreach (App\JournalRecordsTemp::where('account_head_id', $unique_acc_head->account_head_id )->whereBetween('journal_date', [$from,$to])->get() as $record)
                                    
                                    @php
                                        $reverse= $record->transaction_type== "DR" ? "CR" : "DR";   
                                    @endphp
                                    @foreach (App\JournalRecordsTemp::where('journal_temp_id',$record->journal_temp_id)->where('transaction_type', $reverse)->whereBetween('journal_date', [$from,$to])->get() as $ledger_record)
                                    <tr>
