        ...
        <div class="content-wrapper">
            <div class="content-body">
                ...
                <section id="widgets-Statistics">
                            ...
                            <form action="{{ route('searchStockPositionDateP') }}" method="GET">
                                ...
                                <div class="row form-group col-right-padding">
                                    <input type="text" class="form-control col-9" name="date"
                                        placeholder="Select Date" onfocus="(this.type='date')" id="date" value="{{ $searchDate }}" required>
                    ...
                    <div class="row pt-2">
                        <div class="col-md-12">
                            <a href="{{ route('printStockPositionDateP',$date) }}" class="btn btn-sm btn-info float-right"
                            target="_blank">Print</a>
                            <button class="btn  btn-info btn-sm float-right mr-1"
                        onclick="exportTableToCSV('stockposition-{{ $date }}.csv')">Export To CSV</button>
                        </div>
                        ...
                        @if(1==1)
                                ...
                                @endphp
                                    <tr>
                                        ...
                                        <td>{{$itmXsC= isset($itmXs)? $itmXs->datePurchase($date):0 }}</td>
                                        <td>{{ $itmSC =isset($itmS)? $itmS->datePurchase($date):0 }}</td>
                                        <td>{{$itmMC= isset($itmM)? $itmM->datePurchase($date):0 }}</td>
                                        <td>{{$itmLC= isset($itmL)? $itmL->datePurchase($date):0 }}</td>
                                        <td>{{$itmXlC= isset($itmXl)? $itmXl->datePurchase($date):0 }}</td>
                                        <td>{{$itmXxlC= isset($itmXxl)? $itmXxl->datePurchase($date):0 }}</td>
                                        <td>{{ $itmXxxlC=isset($itmXxxl)? $itmXxxl->datePurchase($date):0 }}</td>
                                        <td>{{  $itmXsC+$itmSC+$itmMC+$itmLC+$itmXlC+$itmXxlC+$itmXxxlC }}</td>
                                    </tr>
                                    @php
